 ...
 *
 * Unlike the app this replaces there is no local-only mode, so both blank is a
 * broken build rather than a lesser one — a guide lives in the account that
 * owns the trip and nowhere else. It is allowed locally, where somebody may
 * legitimately not have the secrets, and refused in CI, where shipping it
 * would deploy a site that cannot open anything.
 */
...
/**
 * Unsplash's access key is a public client id, like the Supabase anon key: it
 * identifies the application and authorises nothing on anybody's behalf. It
 * ships in the bundle because the photo swap runs in the browser, and the
 * secret key it is paired with never leaves the routine.
 */
...
if (process.env['CI'] && !supabaseUrl) {
  throw new Error(
    'No PUBLIC_SUPABASE_URL in CI. This would deploy a site that cannot open a ' +
      'guide, because there is no local-only mode here.',
  )
}
...
const workerCode = new TextDecoder()
  ...
  .replace('__CACHE_NAME__', `cicerone-${build}-${hash}`)
...
console.log(
  `dist/ built \u2014 ${buildLabel(build)}, ${bundleName} ` +
    ...
    `backend ${supabaseUrl ? 'configured' : 'NOT configured'}, ` +
    `photos ${unsplashKey ? 'configured' : 'off'}`,
)
