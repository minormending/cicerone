 ...
 *   node scripts/db.mjs status  cicerone        this app's own schema
 *   node scripts/db.mjs migrate cicerone
...
const SETS = {
  cicerone: { dir: 'supabase/cicerone', schema: 'cicerone' },
...
async function withSet(name, fn) {
  ...
  }, { root: ROOT, applicationName: `cicerone/db:${name}` })
}
...
switch (command) {
  ...
  case 'queue':
    await withClient(
      ...
      { root: ROOT, applicationName: 'cicerone/db:queue' },
    )
    break
  ...
  case 'query': {
    ...
    await withClient(async (client) => printResult(await client.query(sql)), {
      root: ROOT,
      applicationName: 'cicerone/db:query',
    })
    break
  }
