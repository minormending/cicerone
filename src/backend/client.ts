...
/**
 * Injected at build time. Both blank means no backend is configured, which
 * here is a broken build rather than a mode: unlike the app this replaced,
 * there is no local-only path. A guide lives in the account that owns the trip
 * and nowhere else, so the reader says so plainly rather than half-working.
 */
...
function make(url: string, key: string) {
  return createClient(url, key, {
    ...
    // This app owns the `cicerone` schema. `public` holds the shared platform
    // layer every app in this database uses: profiles, apps, rate_limit.
    db: { schema: 'cicerone' },
  })
}
